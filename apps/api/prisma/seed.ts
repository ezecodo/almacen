...
async function main() {
  ...
  for (const rest of restaurantes) {
    const restaurant = await prisma.restaurant.create({
      data: {
        nombre: rest.nombre,
        empleados: {
          create: rest.empleados.map((nombre) => ({ nombre })),
        },
      },
      include: { empleados: true },
    });
    creados.push(restaurant);
  ...
  for (let i = 0; i < creados.length; i++) {
    const rest = creados[i];
    const diasRetiros = retirosPorRestaurante[i].dias;

    for (const dia of diasRetiros) {
      const empleado =
        rest.empleados[Math.floor(Math.random() * rest.empleados.length)];
