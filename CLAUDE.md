...
7. **Modo local sin internet — solo Sala/TPV** (idea 2026-09-21, roadmap sin implementar todavía): Eze retoma la necesidad de que la sala siga funcionando si se corta el servicio de internet del restaurante (la WiFi local con la Pi sigue viva, solo se cae el enlace del router al ISP). **Importante**: esto es la misma arquitectura "Offline-First" descartada el 2026-09-16 más arriba — se reabre ahora porque Eze mismo la pide explícitamente (antes era una propuesta externa sin pedido concreto). La diferencia clave para que esto sea viable sin repetir el salto de alcance de la vez pasada: **acotar el modo local SOLO al flujo de Sala/TPV** (abrir mesa, comandar, imprimir) — admin, staffing, reservas, inventario, wiki, checklists, etc. pueden quedar inaccesibles durante el corte sin drama; lo único que no puede parar a mitad de un servicio es tomar pedidos y que salgan a cocina/barra.
